...
class ControlTest : public ::testing::Test {
 protected:
  virtual void SetUp() { 
    platform_fopen(&outf, __FILE__".log", "w");
  }
...
class BangBang1Test : public ControlTest {
 protected:
  BangBang1 traj;
...
TEST_F(BangBang1Test, positive) {
  x0 = -2.f, xf = 2.f;
  EXPECT_TRUE(traj.reset(period, 0.01f, x0, 0, xf, max_vel, max_acc, max_dec));
  EXPECT_EQ(traj.pos, x0);
  EXPECT_EQ(traj.vel, 0);
  EXPECT_EQ(traj.acc, max_acc);
  if(outf) fprintf(outf, "%f, %f, %f\n", traj.acc, traj.vel, traj.pos);
  traj.next();
  ...
  traj.next();
  if(outf) fprintf(outf, "%f, %f, %f\n", traj.acc, traj.vel, traj.pos);
  EXPECT_LT(fabs(traj.pos - xf), 1E-6);
  EXPECT_EQ(traj.vel, 0);
  EXPECT_EQ(traj.acc, 0);
}
