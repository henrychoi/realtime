#include "platform/all.h"//Must be the first include!!!!!
#include <stdio.h>
#include <math.h>
#include <float.h>
#include <algorithm>
...
bool ScalarLowpass::reset(float initial, float hz, float pd
  ...
  // Sanity check
  if(order >= window) {
    snprintf(msg, sizeof(msg), "Filter order %d >= window %d"
      , order, window);
    log_fatal(NULL, msg);
    ok = false;
  }
  ...
  if(order) {
    if(cutoffhz > 1/period) {/* Cut-off frequency should be at most
      ...
      snprintf(msg, sizeof(msg)
        , "Cutoff freq %f > 10%% * sampling rate %f\n" , cutoffhz, 1/period);
      log_fatal(NULL, msg);
      ok = false;
      ...
      snprintf(msg, sizeof(msg)
        , "Cutoff freq %f < 0.1%% * sampling rate %f\n", cutoffhz, 1/period);
      log_fatal(NULL, msg);
      ok = false;
    }
  }
  ...
  // Compute coeff
  float alpha = (float)exp(-2.0f * M_PI * cutoffhz * period);
  ...
  default:
    snprintf(msg, sizeof(msg)
      , "Estimator order %d unimplemented\n", order);
    log_fatal(NULL, msg);
    ok = false;
  }
