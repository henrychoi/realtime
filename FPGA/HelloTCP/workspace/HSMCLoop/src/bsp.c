...
#if BSP_TICKS_PER_SEC < 5 // I can't see the faster blinks anyway
...
void BSPConsole_print(const char* reply, uint16_t msgBufferLen
		, void* replyParam) {
	struct tcp_pcb* tpcb = (struct tcp_pcb*)replyParam;
	u16_t replyLen = strlenn(reply, msgBufferLen);
	replyLen = MIN(replyLen, tcp_sndbuf(tpcb));
	tcp_write(tpcb, reply, replyLen, TCP_WRITE_FLAG_COPY);
}
extern void onPacket(char* payload, uint16_t len
    , BSPConsoleReplyFn replyFn, void* replyParam);

err_t echo_onrecv(void* a, struct tcp_pcb* tpcb, struct pbuf*p, err_t err) {
	...
	onPacket(p->payload, p->len, BSPConsole_print, tpcb);
	pbuf_free(p);/* free the received pbuf */

	return ERR_OK;
}
/*..........................................................................*/
err_t echo_onaccept(void *arg, struct tcp_pcb *tpcb, err_t err) {
...
#define REPLY_BUFFER_SIZE 1024
	...
	tcp_recv(tpcb, echo_onrecv);

	/* just use an integer number indicating the connection id as the
	   callback argument */
	tcp_arg(tpcb, (void*)connection);
    sprintf(reply, "%s\n%s", USAGE, PROMPT);
    BSPConsole_print(reply, REPLY_BUFFER_SIZE, tpcb);
...
#  if 1
	{
		...
		IP4_ADDR(&qspyHost,  192, 168,   0, 14);
