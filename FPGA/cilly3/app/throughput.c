...
int __cdecl main(int argc, char *argv[]) {
  ...
  rawf = fopen("cl_raw.bin", "wb");
  if(!rawf) {
    fprintf(stderr, "Failed to open cl_raw.txt\n");
    exit(1);
  }
  ...
  n_frame = atoi(argv[1]);
  n_frame |= 0x100000; // frame the message with the command itself
  allwrite(write_fd, &n_frame, sizeof(n_frame));

  //read_bytes = _read(loop_fd, buf, sizeof(buf));

  startTick = GetTickCount();
  for(cur_frame = 0; cur_frame < n_frame; ++cur_frame) {
    for(n_bytes = 0; ; ) {
      void* ptr;
      size_t written;
      ...
      //printf(".");
      if (!do_bytes) {
        printf("\nXillybus read stream finished; exiting.\n");
        goto done;
      }
      for(ptr = info.addr, written = 0; written < do_bytes; ) {
        size_t c = fwrite((char*)info.addr + written, 1, do_bytes - written, rawf);
        if(!c) {
          printf("\nRaw output file closed; exiting.\n");
          goto done;
        }
        written += c;
      }
