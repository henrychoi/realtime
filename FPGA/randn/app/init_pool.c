...
#define N_PATCH 600000//(1<<LFSR_SIZE)
  ...
  float randn;
  unsigned int n_msg, *prandn = (unsigned int*)(&randn);
  FILE *pool_f = fopen("c:/private/realtime/FPGA/randn/doc/Pool1024.csv", "r")
    , *bin_pool_f = NULL;
  if(!pool_f) {
    perror("Failed to open pool data file");
    exit(errno);
  }
  ...
  if(bTalk2FPGA) {
    ...
    bin_pool_f = fopen("bin_pool.bin", "wb");
    if(!bin_pool_f) {
      perror("Failed to open binary pool data file to write");
      exit(errno);
    }
  }

  for(n_msg = 0; fscanf(pool_f, "%f", &randn) == 1; ++n_msg) {
    printf("%f => 0x%08X\n", randn, *prandn);
    if(bTalk2FPGA) {
      allwrite(write_fd, (unsigned char*)prandn, sizeof(*prandn));
    } else {
      fwrite(prandn, sizeof(*prandn), 1, bin_pool_f);
    }
  } //end for
...
cleanup:
  if(bTalk2FPGA) {
    ...
    if(bin_pool_f) fclose(bin_pool_f);
  }
