...
static interrupt void cpu_timer0_isr(void) {
    ...
    DEBUG_LED_toggle();
	//EQep1Regs.QEPCTL.bit.SWI = 1;//pulse the position counter
	//EQep1Regs.QEPCTL.bit.SWI = 0;//need to clear this explicitly
}
...
void BSP_init(void) {
	...
	EALLOW; //Configure eQEP1 to count pulses in HW
	GpioCtrlRegs.GPAPUD.bit.GPIO20 = 0;//enable pullup on eQEP A (pulse)
	GpioCtrlRegs.GPAPUD.bit.GPIO21 = 0;//enable pullup on eQEP B (direction)
	...
	GpioCtrlRegs.GPAMUX2.bit.GPIO20 = 1;//ePWM1 A
	GpioCtrlRegs.GPAMUX2.bit.GPIO21 = 1;//ePWM1 B
	GpioCtrlRegs.GPAMUX2.bit.GPIO22 = 1;//ePWM1 I
	GpioCtrlRegs.GPAMUX2.bit.GPIO23 = 1;//ePWM1 S

    EQep1Regs.QUPRD=1500000; // Unit Timer for 100Hz at 150MHz SYSCLKOUT
    EQep1Regs.QDECCTL.bit.QSRC = 2;//Direction count mode; also tried 1
    EQep1Regs.QDECCTL.bit.XCR = 1;// 1x resolution (rising edge only)
	//EQep1Regs.QEPCTL.bit.FREE_SOFT = 2;//registers unaffected by emulation suspend
	...
	//QEP watchdog, capture, timer are implicitly disabled on reset
	EDIS;
